...
const SkillCard = ({ name, level, icon: Icon, category, delay = 0 }) => {
  ...
  return (
    <div
      className={`group relative rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border-2 ${categoryStyles[category]} animate-fade-in-up`}
      style={{ animationDelay: `${delay}ms` }}
    >
      <div className="flex items-center justify-between mb-4">
        <div className={`p-3 rounded-lg bg-gradient-to-br ${categoryStyles[category]} group-hover:scale-110 transition-transform duration-300`}>
          <Icon size={24} />
        </div>
